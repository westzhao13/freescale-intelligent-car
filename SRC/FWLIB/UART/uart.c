...
void UART_INIT()
{
  ...
  uint16_t sbr;
   ...
   UART_DebugPort=UART5;
	...
	UART_PORT = PORTE;
	UART_PORT->PCR[8] &= ~PORT_PCR_MUX_MASK;
	UART_PORT->PCR[8] |= PORT_PCR_MUX(3);
	UART_PORT->PCR[9] &= ~PORT_PCR_MUX_MASK;
	UART_PORT->PCR[9] |= PORT_PCR_MUX(3);
  ...
  clock = CPUInfo.BusClock;
	...
	sbr = (uint16_t)((clock)/((115200)*16));
	brfa = ((clock*2)/(115200)-(sbr*32));
